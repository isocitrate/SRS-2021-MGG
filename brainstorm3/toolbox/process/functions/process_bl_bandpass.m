...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.options.sensortypes.Value   = 'Grad';
    sProcess.options.sensortypes.InputTypes = {'data', 'raw'};
    % ==== Parameters 
    sProcess.options.label1.Comment = '<BR><B>Recommended filtering parameters</B>:';
    sProcess.options.label1.Type    = 'label';

    sProcess.options.label3.Comment = '<U>Cardiac</U>: 1 Hz &le f &le 60 Hz &nbsp &nbsp &nbsp <U>Gastric</U>: 0 Hz &le f &le 0.1 Hz <BR><BR>' ;
...
function [Comment, fileTag] = FormatComment(sProcess)
    ...
    if ~isempty(HighPass) && ~isempty(LowPass)
        Comment = ['Band-pass (BP):' num2str(HighPass) 'Hz-' num2str(LowPass) 'Hz'];
        fileTag = 'Band-pass';
    elseif ~isempty(HighPass)
        Comment = ['High-pass (HP):' num2str(HighPass) 'Hz'];
        fileTag = 'high';
    elseif ~isempty(LowPass)
        Comment = ['Low-pass (LP):' num2str(LowPass) 'Hz'];
        fileTag = 'low';
    else
        Comment = '';
    end
...
function sInput = Run(sProcess, sInput) %#ok<DEFNU>
    ...
    % So in order for everything to work the way I want it to, it could be
    % necessary for me to re-reference sInput so the pipeline works.
    % Otherwise this code doesn't really run properly for a pipeline. 
    clc;
    clear sOutput;
    ProtocolInfo = bst_get('ProtocolInfo');
    load(strcat(ProtocolInfo.STUDIES, '/', sInput.ChannelFile), 'Channel')
    ...
    gradID = strcmpi(ChannelType, 'Grad');
    filtData = Data(gradID,:);

    %sfreq = 1 ./ (sInput.TimeVector(2) - sInput.TimeVector(1));
    [filtData, FiltSpec, Messages] = Compute(filtData, sfreq, HighPass, LowPass, Method, isMirror, isRelax, TranBand);

    Data(gradID,:) = filtData;
    % Process warnings
    if ~isempty(Messages)
    ...
    % File comment
    if ~isempty(HighPass) && ~isempty(LowPass)
        filterComment = ['bandpass(' num2str(HighPass) '-' num2str(LowPass) 'Hz)'];
    elseif ~isempty(HighPass)
...
%% Write Results
    ...
    if contains(filetag, 'Band-pass')
        RawFileName = strcat(ProtocolDataDir, '/', FileDir, '/', ...
                           StudyName, '_BP_', num2str(HighPass),'to', num2str(LowPass), 'Hz', '.raw');
    elseif contains (filetag, 'high')
        RawFileName = strcat(ProtocolDataDir, '/', FileDir, '/', ...
                           StudyName, '_HP_', num2str(HighPass), 'Hz', '.raw');
    elseif contains (filetag, 'low')
        RawFileName = strcat(ProtocolDataDir, '/', FileDir, '/', ...
            StudyName, '_LP_', num2str(LowPass), 'Hz', '.raw');
    else
        RawFileName = strcat(ProtocolDataDir, '/', FileDir, '/', ...
            StudyName, '.raw');
    end 
    DataOut = Data;
    clear Datamat;
    datamat.data=DataOut';
    datamat.sf=sfreq;
    datamat.Comment= sprintf('data_%s', filterComment);
    [datamat]=CreatedbBrainstorm(datamat, 1:size(datamat.data,2));
